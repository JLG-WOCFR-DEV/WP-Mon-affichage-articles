...
if ( ! function_exists( 'my_articles_normalize_color_value' ) ) {
    /**
     * Normalizes a color value to a CSS-ready string.
     *
     * Accepts hex values with or without the leading hash as well as RGB(A)
     * declarations. Returns an empty string for unsupported formats.
     *
     * @param mixed $value Raw color value.
     *
     * @return string
     */
    function my_articles_normalize_color_value( $value ) {
        if ( ! is_string( $value ) ) {
            return '';
        }
...
if ( ! function_exists( 'my_articles_resolve_theme_color_reference' ) ) {
    /**
     * Resolves a theme color reference to a concrete CSS color value.
     *
     * Supports direct color strings as well as "var:preset|color|slug"
     * references generated by the Global Styles engine.
     *
     * @param mixed $value        Color reference value.
     * @param array $palette_map  Associative array of palette colors indexed by slug.
     *
     * @return string
     */
    function my_articles_resolve_theme_color_reference( $value, array $palette_map ) {
        if ( ! is_string( $value ) ) {
            return '';
        }
        ...
        if ( 0 === strpos( $value, 'var:' ) ) {
            $reference = substr( $value, 4 );
            $parts     = explode( '|', $reference );
            $slug      = '';

            if ( count( $parts ) >= 3 && 'color' === $parts[0] ) {
                $slug = $parts[2];
            } elseif ( ! empty( $parts ) ) {
                $slug = end( $parts );
            }
            ...
            if ( $slug && isset( $palette_map[ $slug ] ) ) {
                return $palette_map[ $slug ];
            }

            return '';
        }

        return my_articles_normalize_color_value( $value );
    }
}

if ( ! function_exists( 'my_articles_get_theme_preview_tokens' ) ) {
    /**
     * Retrieves theme color tokens suitable for admin previews.
     *
     * @return array{
     *     background: string,
     *     foreground: string,
     ...
     * }
     */
    function my_articles_get_theme_preview_tokens() {
        static $cached_tokens = null;

        if ( null !== $cached_tokens ) {
        ...
        if ( function_exists( 'wp_get_global_settings' ) ) {
            $palette_entries = wp_get_global_settings( array( 'color', 'palette', 'theme' ) );

            if ( is_array( $palette_entries ) ) {
                foreach ( $palette_entries as $entry ) {
                    ...
                    $color = isset( $entry['color'] ) ? my_articles_normalize_color_value( $entry['color'] ) : '';

                    if ( $slug && $color ) {
                        $tokens['palette'][ $slug ] = $color;
                    }
                }
            }

            $background = wp_get_global_settings( array( 'color', 'background' ) );
            $foreground = wp_get_global_settings( array( 'color', 'text' ) );

            $resolved_background = my_articles_resolve_theme_color_reference( $background, $tokens['palette'] );
            $resolved_foreground = my_articles_resolve_theme_color_reference( $foreground, $tokens['palette'] );

            if ( $resolved_background ) {
                $tokens['background'] = $resolved_background;
            }

            if ( $resolved_foreground ) {
                $tokens['foreground'] = $resolved_foreground;
            }
        }

        if ( '' === $tokens['background'] ) {
            foreach ( array( 'background', 'base', 'body', 'surface' ) as $candidate ) {
                if ( isset( $tokens['palette'][ $candidate ] ) ) {
                    $tokens['background'] = $tokens['palette'][ $candidate ];
                    break;
                }
            }
        }
        ...
        if ( '' === $tokens['foreground'] ) {
            foreach ( array( 'contrast', 'foreground', 'text' ) as $candidate ) {
                if ( isset( $tokens['palette'][ $candidate ] ) ) {
                    $tokens['foreground'] = $tokens['palette'][ $candidate ];
                    break;
                }
            }
        }
        ...
        if ( '' === $tokens['background'] ) {
            ...
            if ( is_string( $custom_background ) && '' !== $custom_background ) {
                $normalized = my_articles_normalize_color_value( $custom_background );

                if ( $normalized ) {
                    $tokens['background'] = $normalized;
                }
            }
        }

        if ( '' === $tokens['foreground'] ) {
            $custom_text_color = get_theme_mod( 'header_textcolor' );

            if ( is_string( $custom_text_color ) && '' !== $custom_text_color && 'blank' !== $custom_text_color ) {
                $normalized = my_articles_normalize_color_value( $custom_text_color );

                if ( $normalized ) {
                    $tokens['foreground'] = $normalized;
                }
            }
        }
...
public function prepare_filter_articles_response( array $args ) {
    ...
    public function render_admin_preview() {
        ...
        $theme_tokens = my_articles_get_theme_preview_tokens();
