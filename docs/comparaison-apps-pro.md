...
| **Moteur de requête** | Filtres taxonomiques, recherches plein texte, meta queries AND/OR et adaptateurs externes configurables.【F:mon-affichage-article/includes/class-my-articles-shortcode.php†L1237-L1305】【F:mon-affichage-article/includes/class-my-articles-shortcode.php†L951-L1083】 | Meta queries imbriquées, pipelines ElasticSearch/Algolia pilotés graphiquement. | Proposer un builder visuel (groupes imbriqués, prévisualisation) et des connecteurs prêts à l’emploi pour les index distants. |
...
3. **Moteur de requête**
   - Le panneau « Requêtes avancées » permet désormais de combiner des meta queries (`AND`/`OR`) et d'agréger des contenus externes via des adaptateurs PHP, mais l'expérience reste textuelle (JSON) et ne gère pas encore les groupements imbriqués ni la validation interactive attendue dans les outils premium.【F:mon-affichage-article/includes/class-my-articles-metaboxes.php†L560-L626】【F:mon-affichage-article/includes/class-my-articles-shortcode.php†L951-L1083】
...
3. **Requêtes avancées et connecteurs**
   - Structurer une interface guidée autour du nouveau panneau (modèles de clauses, validation en temps réel, prévisualisation des résultats) et enrichir le registre d’adaptateurs avec des connecteurs certifiés (ElasticSearch, Matomo, API internes) prêts à l’emploi.【F:mon-affichage-article/includes/class-my-articles-metaboxes.php†L560-L626】【F:mon-affichage-article/includes/class-my-articles-shortcode.php†L951-L1083】
...
6. **Connecteurs externes & requêtes expertes**
   - Documenter une API dédiée (`my_articles_register_content_adapter()`) pour exposer des collections ElasticSearch, headless CMS ou flux marketing directement dans la boucle de rendu, avec un fallback pour les hooks historiques.【F:mon-affichage-article/includes/helpers.php†L233-L315】【F:mon-affichage-article/includes/class-my-articles-shortcode.php†L776-L824】
   - Prévoir des formulaires d’édition structurés (metabox, panneau Gutenberg) capables de sérialiser des `meta_query` complexes (types, opérateurs, relation `AND/OR`) et de guider les équipes sur les adaptateurs disponibles.【F:mon-affichage-article/includes/class-my-articles-metaboxes.php†L338-L420】【F:mon-affichage-article/blocks/mon-affichage-articles/edit.js†L2318-L2407】
